...
# ** 2ページ目A (運用改善系) **
    ...
    with st.form("input_form"):
        ...
        # **事前定義された値**
    predefined_values = [
        ("電気の排出係数", 0.000434 if st.session_state["user_input"].get("燃料") == "電力" else 0.0, "t-CO2/kWh", "・環境省令和5年：0.000434(t-CO2/kWh)\nhttps://ghg-santeikohyo.env.go.jp/files/calc/r05_coefficient_rev4.pdf" if st.session_state["user_input"].get("燃料") == "電力" else ""),
        ("電気料金", 22.97 if st.session_state["user_input"].get("燃料") == "電力" else 0.0, "円/kWh", "・新電力ネット(高圧)22.97(円/kWh)\nhttps://pps-net.org/unit" if st.session_state["user_input"].get("燃料") == "電力" else ""),
        ("想定稼働年数", 10, "年", "")
    ]
    ...
    for name, value, unit, description in predefined_values:
        ...
        # ユーザー入力欄
        st.session_state["user_input"][name_key] = st.text_input(
            f"規定値({name})の名前", value=st.session_state["user_input"][name_key]
        )
        st.session_state["user_input"][num_key] = st.number_input(
            f"規定値({name})の数字",
            min_value=0.0,
        ...
        st.session_state["user_input"][unit_key] = st.text_input(
            f"規定値({name})の単位", value=st.session_state["user_input"][unit_key]
        )
        st.session_state["user_input"][desc_key] = st.text_area(
            f"規定値({name})の説明", value=st.session_state["user_input"][desc_key]
        )
        # **追加の規定値 13個**
        for i in range(13):
