...
# ** 2ページ目A (運用改善系) **
    ...
    for name, value, unit, description in predefined_values:
        ...
        # 数値が None の場合はデフォルトで 0.0 にせず、空欄にする
        if value is None:
            numeric_value = 0.0
            disabled = True
        else:
        ...
        st.session_state["user_input"][f"規定値_{name}_単位"] = st.text_input(f"規定値({name})の単位", value=unit if value is not None else "", disabled=disabled)
        st.session_state["user_input"][f"規定値_{name}_説明"] = st.text_area(f"規定値({name})の説明", value=description if value is not None else "", disabled=disabled)
